...
The subagent MUST call the plugin via Python inline. Do NOT use the `image_generate` tool — it doesn't support all plugin parameters.
...
result = provider.generate(
    ...
    aspect_ratio='1:1',
    r2_name='nombre_custom',  # opcional, snake_case
    storage='r2'  # 'r2' (permanente) o 'local'
)
print(result)
"
```
...
- Image generation — fast (timeout 30s), no polling needed
...
2. If user provides a custom name, use it. Otherwise auto-generate name as above
3. Call `image_generate` tool with prompt, model, aspect_ratio
4. If a custom name was provided or auto-generated, pass it as `r2_name` parameter to organize the R2 upload (e.g., `image_generate(..., r2_name="cuphead_nivel_imaginario")`)
5. Register in `image_registry.json`
